...
- **Scout Elevated V2 PR1** (`scout_interview_sessions` / `scout_captured_units`, `routers/scout.py`): a founder/SME interview session, a live-editable Work Capture Grid, and a Genome Strength completeness meter are real — tenant-scoped via `X-Spec-Key`/RLS, browser-tested end to end (session create → 401 → key banner → retry → add row → live meter update → inline edit → persisted). **Not built in PR1**, despite being named in the design doc: the 5 "elevation" modules (Time-Travel Replay, Contradiction Resolver, Pain Heatmap, Story-to-Structure live trace, Future Preview), the `/scout/sessions/{id}/transcript` and `/scout/extract-from-story` endpoints, and `scout_contradictions`/`scout_pain_signals` tables. None of the "adaptive AI follow-up" or "words become structure live" behavior the design doc describes runs on an LLM — `LLM_PROVIDER=none` in this environment. `DiscoveryPartner`'s question bank is a static, hand-written list per track (founder/SME), not AI-generated or AI-adaptive; its voice input is the browser's own `SpeechRecognition` API (dictation only, works with no backend LLM, degrades to typing where unsupported) — it is not an AI interviewer. Completeness is computed from structured fields the interviewer/interviewee types or dictates into the grid directly; nothing extracts structure from free-text notes automatically. The design doc's `org_id` naming was corrected to `client_id` throughout (this repo's actual tenant column — see `SCOUT_OPERATING_PLAYBOOK.md` §A.3, "never introduce an `orgs` table").
