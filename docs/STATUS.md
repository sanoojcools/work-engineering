...
| **Ship-ready (hosted product)** | **Hosted and live; "Set up the demo" now actually works** | The Render free-tier blueprint (`render.yaml`) is deployed and starts clean, but "clean startup + `200` health checks" was not the same claim as "works": `POST /api/demo/bootstrap` 500'd on Render specifically, because the managed database's owning role isn't a real Postgres superuser (confirmed via `pg_roles`) the way docker-compose's same-named role always is locally — see `docs/HONESTY.md` for the full root cause and fix, verified by reproducing Render's exact role attributes against a real local Postgres. `main.py`'s startup `bootstrap_tenants()` call had the identical, silent version of the same bug (`db_ready` could read `true` while that step had actually failed). Both are fixed and re-verified against Render-matching conditions, not just re-deployed on faith. CI now exists (`.github/workflows/ci.yml`) — real Postgres-backed backend suite plus frontend typecheck/build on every push/PR. The PII-encryption key no longer has a hardcoded default — `PII_ENCRYPTION_KEY` is required, unset in any environment refuses to start. Still missing: monitoring, backup, a load test, a browser walk of the hosted URL itself (still proxy-restricted from this environment), and everything in tracks 4–6 of the enterprise-readiness roadmap. |
