...
| **Ship-ready (hosted product)** | **Not yet, closer** | CI now exists (`.github/workflows/ci.yml`) — real Postgres-backed backend suite plus frontend typecheck/build on every push/PR; none existed before. The PII-encryption key no longer has a hardcoded default — `PII_ENCRYPTION_KEY` is required, unset in any environment refuses to start rather than silently encrypting with a known key. Still missing: monitoring, backup, a load test, and everything in tracks 4–6 of the enterprise-readiness roadmap. |
