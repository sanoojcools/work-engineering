...
def unit_to_work_unit_import(unit: ScoutCapturedUnit, interviewee_name: str) -> dict:
    return {
        "id": _wu_id(unit),
...
def build_genome_payload(session: ScoutInterviewSession) -> dict:
    return {
        "work_units": [unit_to_work_unit_import(u, session.interviewee_name) for u in session.units],
    }


def generate_genome(db: Session, session: ScoutInterviewSession, *, actor: str) -> dict:
    payload = build_genome_payload(session)
    return import_genome(db, session.client_id, payload, actor=actor)
