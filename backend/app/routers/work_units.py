...
def create_work_unit(payload: WorkUnitCreate, db: DbDep) -> WorkUnitOut:
    try:
...
def reconcile(work_unit_id: int, db: DbDep) -> WorkUnitOut:
    wu = get_or_404(db, WorkUnit, work_unit_id, "WorkUnit")
    try:
...
def authoritative(work_unit_id: int, db: DbDep) -> WorkUnitOut:
    wu = get_or_404(db, WorkUnit, work_unit_id, "WorkUnit")
    passed = (
...
def create_variant(work_unit_id: int, payload: VariantCreate, db: DbDep) -> WorkUnitVariant:
    wu = get_or_404(db, WorkUnit, work_unit_id, "WorkUnit")
...
def list_variants(work_unit_id: int, db: DbDep) -> Page[VariantOut]:
...
def promote(work_unit_id: int, payload: PromoteIn, db: DbDep) -> AutonomyChangeOut:
    wu = get_or_404(db, WorkUnit, work_unit_id, "WorkUnit")
    try:
...
def demote(work_unit_id: int, payload: DemoteIn, db: DbDep) -> AutonomyChangeOut:
    wu = get_or_404(db, WorkUnit, work_unit_id, "WorkUnit")
    try:
